...
function createHarness(options = {}) {
  ...
  const claimHistoryPanel = {
    get innerText() {
      return [state.claimHistoryTitleText, state.claimHistoryNewestDateText]
        .filter(Boolean)
        .join("\n");
    },
    parentElement: null,
  ...
  const document = {
    ...
    querySelectorAll(selector) {
      ...
      if (selector === "*") {
        return state.claimHistoryOpen
          ? [claimHistoryTitle, claimHistoryPanel, claimHistoryDate]
          : [];
      }
      return [];
    },
...
test("getNewestClaimHistoryDate uses newest date not first date", () => {
  const { context } = createHarness({
    claimHistoryOpen: true,
    claimHistoryTitleText: "Claim History",
    claimHistoryNewestDateText:
      "2026-05-14 08:00:00\n2026-05-16 09:00:00\n2026-05-15 10:00:00",
  });

  assert.equal(context.getNewestClaimHistoryDate("2026-05-16"), "2026-05-16");
});
...
test("does not report success when newest claim date is not today", () => {
  ...
  assert.equal(state.timeoutDelays.filter((delay) => delay === 5000).length, 2);
});
