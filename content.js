...
const CLAIM_RECORD_SELECTORS = [
  CLAIM_RECORD_SELECTOR,
  '[aria-label*="領取紀錄"]',
  ...
  '[title*="Claim History"]',
  '[title*="Record"]',
  ".sc-dhTHNW.sc-dxAgDQ.fnLAlO.hGFFwq",
  "#main > div.sc-hqTDhr.isvhpF > div > div.sc-eDOMzu.vohmc > div.sc-fbUgXY.JDvFt > div.sc-dhTHNW.sc-dxAgDQ.fnLAlO.hGFFwq",
];
const DAY_LABEL_REGEX = /^Day\s+\d+$/;
const CLAIM_HISTORY_TITLE_TEXTS = ["領取紀錄", "Claim History"];
const CLAIM_HISTORY_BUTTON_TEXTS = [
  "領取紀錄",
  "Claim History",
  "Record",
  "History",
...
function findSignInButton() {
  ...
  if (!lottie) {
    ...
    if (dayLabelButton) {
      console.log(
        "Found clickable day label:",
        dayLabelButton.innerText?.trim(),
      );
      return dayLabelButton;
    }

    console.log(
      "No lottie-container found - either already signed in or page not loaded",
    );
    return null;
  }
  ...
  for (let i = 0; i < 10 && current; i++) {
    ...
    if (
      ...
      current.className.includes("sc-nuIvE")
    ) {
    ...
    if (style.cursor === "pointer") {
      console.log(
        "Found clickable parent via cursor:pointer:",
        current.className,
      );
      return current;
    }
...
function hasNoGameCharacterToast() {
  ...
  return (
    toastText.includes(NO_GAME_CHARACTER_TEXT) ||
    bodyText.includes(NO_GAME_CHARACTER_TEXT)
  );
}
...
function hasClaimRecordSection() {
  ...
  return (
    Boolean(document.querySelector(CLAIM_RECORD_SELECTOR)) ||
    bodyText.includes("領取紀錄")
  );
}
...
function extractAllDatesFromText(text) {
  ...
  for (const match of normalizedText.matchAll(
    ...
    dates.push(normalizeDateParts(match[1], match[2], match[3]));
  }
...
function extractDateFromText(text) {
  return extractAllDatesFromText(text)[0] || null;
}
...
function findClaimHistoryTitleNode() {
  ...
  return (
    nodes.find((node) => {
      ...
      return textIncludesAny(text, CLAIM_HISTORY_TITLE_TEXTS);
    }) || null
  );
}

function bodyLooksLikeOpenClaimHistoryPanel() {
  const bodyText = document.body?.innerText || "";
  return (
    textIncludesAny(bodyText, CLAIM_HISTORY_TITLE_TEXTS) &&
    Boolean(extractDateFromText(bodyText))
  );
}

function isClaimHistoryPanelOpen() {
  return (
    Boolean(findClaimHistoryTitleNode()) || bodyLooksLikeOpenClaimHistoryPanel()
  );
}
...
function getNewestClaimHistoryDate(maxDate = null) {
  const root = getClaimHistoryPanelRoot();
  const sourceRoot = root || document.body;
  const dates = [];

  if (sourceRoot) {
    dates.push(...extractAllDatesFromText(sourceRoot.innerText || ""));
  }

  const nodes =
    sourceRoot && typeof sourceRoot.querySelectorAll === "function"
      ? Array.from(sourceRoot.querySelectorAll("*"))
      : [];
  for (const node of nodes) {
    dates.push(...extractAllDatesFromText(node.innerText?.trim()));
  }
...
function findClaimHistoryButtonByTextOrLabel() {
  const nodes = Array.from(document.querySelectorAll("*"));
  for (const node of nodes) {
    const text = [
      node.innerText || "",
    ...
    if (!textIncludesAny(text, CLAIM_HISTORY_BUTTON_TEXTS)) {
      continue;
    }
...
function findClaimHistorySelectorFallback() {
  for (const selector of CLAIM_RECORD_SELECTORS) {
    const element = document.querySelector(selector);
    if (element) {
      console.log("Found claim history selector fallback:", selector, element);
      return element;
    }
  }
...
function openClaimHistoryPanel() {
  ...
  const labelledButton = findClaimHistoryButtonByTextOrLabel();
  if (labelledButton && clickElement(labelledButton)) {
    return true;
  }

  const fallback = findClaimHistorySelectorFallback();
  if (fallback) {
...
function verifyClaimHistoryWithRetry(remaining = CLAIM_HISTORY_RETRY_COUNT) {
  ...
  const newestDate = panelOpen ? getNewestClaimHistoryDate(today) : null;
  console.log("Claim history verification state:", {
    panelOpen,
    newestDate,
    today,
  });
  ...
  if (remaining > 1) {
    ...
    setTimeout(
      () => verifyClaimHistoryWithRetry(remaining - 1),
      CLAIM_HISTORY_RETRY_DELAY_MS,
    );
    return false;
  }
...
function attemptSignIn() {
  attempts++;
  console.log(
    `Scanning for sign-in button (Attempt ${attempts}/${MAX_ATTEMPTS})...`,
  );

  // Check if today is already signed in
  if (isTodayAlreadyCompleted()) {
    console.log(
      "Today's sign-in is already completed. Verifying claim history before reporting success.",
    );
  ...
  if (target) {
    ...
    setTimeout(() => {
      ...
      showNotification(
        chrome.i18n.getMessage("signInSuccessNotify") || "自動簽到完成",
      );
      setTimeout(() => {
        if (hasNoGameCharacterToast()) {
          console.log(
            "Sign-in failed: no game character found. Leaving tab open.",
          );
          return;
        }
  ...
  if (expandBtn) {
    console.log(
      'Sign-in button not visible yet. Found "View all rewards" button, clicking to expand...',
    );
    expandBtn.click();
    return false;
  }
